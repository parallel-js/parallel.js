(function () {
  ...
  TapReporter.prototype = {
    ...
    reportRunnerResults(runner) {
      ...
      const spec_str = this.executed_specs + (this.executed_specs === 1 ? ' spec, ' : ' specs, ');
      const fail_str = failed + (failed === 1 ? ' failure in ' : ' failures in ');
      const assert_str = this.executed_asserts + (this.executed_asserts === 1 ? ' assertion, ' : ' assertions, ');
