(function () {
	...
	Parallel.prototype.map = function (cb, progress, env) {
		var args = arguments;
		env = (typeof args[1] !== 'function') ? extend(this.options.env, args[1] || {}) : extend(this.options.env, args[2] || {});
		progress = (typeof args[1] === 'function') ? args[1] : function(){};
	...
	Parallel.prototype._spawnReduceWorker = function (data, cb, done, env) {
		...
		if (wrk !== undefined) {
			...
			setImmediate(function () {
				...
				done({data:that.data[that.data.length]});
			});
		} else {
			throw new Error('Workers do not exist and synchronous operation not allowed!');
		}
	...
	Parallel.prototype.reduce = function (cb, progress, env) {
		var args = arguments;
		env = (typeof args[1] !== 'function') ? extend(this.options.env, args[1] || {}) : extend(this.options.env, args[2] || {});
		progress = (typeof args[1] === 'function') ? args[1] : function(){};
		...
		var totalOps = this.data.length - 1;
		var doneOps = 0;
		var that = this;
		function done(msg) {
			var data = msg.data;
