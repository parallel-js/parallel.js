(function () {
	...
	Parallel.prototype.getWorkerSource = function (cb, env) {
		...
		if (isNode) {
			return preStr + 'process.on("message", function(e) {process.send(JSON.stringify((' + cb.toString() + ')(JSON.parse(e).data, ' + env + ')))})';
		} else {
			return preStr + 'self.onmessage = function(e) {self.postMessage((' + cb.toString() + ')(e.data, ' + env +'))}';
		}
	};
	...
	Parallel.prototype.spawn = function (cb, env) {
		...
		env = extend(this.options.environment, env || {});
